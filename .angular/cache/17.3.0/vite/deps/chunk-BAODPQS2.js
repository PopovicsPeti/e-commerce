...
function getNativeRequestAnimationFrame() {
  ...
  let nativeRequestAnimationFrame =
    _global[isBrowser ? "requestAnimationFrame" : "setTimeout"];
  let nativeCancelAnimationFrame =
    _global[isBrowser ? "cancelAnimationFrame" : "clearTimeout"];
  if (
...
var NgZone = class _NgZone {
  constructor({
    ...
    self.nativeRequestAnimationFrame =
  ...
  */
  static isInAngularZone() {
    return (
      typeof Zone !== "undefined" && Zone.current.get("isAngularZone") === true
    );
  }
  /**
...
function afterRender(callback, options) {
  ...
  const instance = runInInjectionContext(
    injector,
    () => new AfterRenderCallback(phase, callback),
  );
...
var ContentChild = makePropDecorator(
  ...
  (selector, opts = {}) =>
    __spreadValues(
      { selector, first: true, isViewQuery: false, descendants: true },
      opts,
    ),
  Query,
);
...
var ViewChild = makePropDecorator(
  ...
  (selector, opts) =>
    __spreadValues(
      { selector, first: true, isViewQuery: true, descendants: true },
      opts,
    ),
  Query,
);
...
function locateOrCreateElementContainerNode(tView, lView, tNode, index) {
  ...
  const hydrationInfo = lView[HYDRATION];
  const isNodeCreationMode =
    !hydrationInfo || isInSkipHydrationBlock$1() || isDetachedByI18n(tNode);
  lastNodeWasCreated(isNodeCreationMode);
  if (isNodeCreationMode) {
    return createCommentNode(lView[RENDERER], ngDevMode ? "ng-container" : "");
  }
...
var Component = makeDecorator(
  "Component",
  (c = {}) =>
    __spreadValues({ changeDetection: ChangeDetectionStrategy.Default }, c),
  Directive,
  void 0,
...
function getEnvironmentInjectorProviders(injector) {
  ...
  for (const providerRecord of providerRecordsWithoutImportPaths) {
    ...
    providerRecords.push(
      __spreadProps(__spreadValues({}, providerRecord), { importPath }),
    );
  }
    ...
    while (true) {
      if (runs === MAXIMUM_REFRESH_RERUNS) {
        throw new RuntimeError(
          103,
          ngDevMode &&
            "Infinite change detection while refreshing application views. Ensure afterRender or queueStateUpdate hooks are not continuously causing updates.",
        );
      }
      if (refreshViews) {
...
_NgZoneChangeDetectionScheduler.ɵfac =
  function NgZoneChangeDetectionScheduler_Factory(t) {
...
function logOversizedImageWarning(src) {
  console.warn(
    formatRuntimeError(
      -913,
      `An image with src ${src} has intrinsic file dimensions much larger than its rendered size. This can negatively impact application loading performance. For more information about addressing or disabling this warning, see https://angular.io/errors/NG0913`,
    ),
  );
}
function internalCreateApplication(config) {
  try {
    ...
    return ngZone.run(() => {
      ...
      const onPlatformDestroyListeners = platformInjector.get(
        PLATFORM_DESTROY_LISTENERS,
      );
...
_ChangeDetectionSchedulerImpl.ɵfac =
  function ChangeDetectionSchedulerImpl_Factory(t) {
...
@angular/core/fesm2022/core.mjs:
   ...
   *)
*/
//# sourceMappingURL=chunk-BAODPQS2.js.map
