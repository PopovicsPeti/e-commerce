...
import { Platform } from "./chunk-S4OX5VLO.js";
...
var _MatToolbar = class _MatToolbar {
  ...
  _checkToolbarMixedModes() {
    if (
      ...
      const isCombinedUsage = Array.from(
        this._elementRef.nativeElement.childNodes,
      )
        .filter(
          (node) =>
            !(node.classList && node.classList.contains("mat-toolbar-row")),
        )
        .filter(
          (node) =>
            node.nodeType !==
...
_MatToolbar.ɵcmp = ɵɵdefineComponent({
  ...
  hostBindings: function MatToolbar_HostBindings(rf, ctx) {
    if (rf & 2) {
      ...
      ɵɵclassProp("mat-toolbar-multiple-rows", ctx._toolbarRows.length > 0)(
        "mat-toolbar-single-row",
        ctx._toolbarRows.length === 0,
      );
    }
  },
  inputs: {
